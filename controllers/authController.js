...
const authController = {
      ...
      return res.status(200).json({
        ...
        data: {
          first_name:
            ...
            users.rows[0].first_name[0].toUpperCase() +
              users.rows[0].first_name.slice(1),
          last_name:
            users.rows[0].last_name &&
            users.rows[0].last_name[0].toUpperCase() +
              users.rows[0].last_name.slice(1),
        },
      });
    } catch (error) {
  ...
  userRegister: async (req, res) => {
    try {
      const { first_name, last_name, email, password, mobile_number } =
        req.body;
      ...
      const user = await db.query(
        `INSERT INTO 
                            ...
                            VALUES 
                                ($1, $2, $3, $4, $5)
                            RETURNING id`,
        [first_name, last_name, email, passwordHarsh, mobile_number]
      );
      //jwt
